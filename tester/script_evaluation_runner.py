...
def get_cmd():
    ...
    if tester_repr == 'WADIFF':
        ...
        print('The result will be saved in :\n\t', Path(result_base_dir) / final_result_repr)
    elif tester_repr == 'WasmSmith':
        # python run_dir_tester_main.py no_mutation_halfdump /media/hdd8T1/inv_wasm_smith/pre_seed/a_default_simplest_rewrite /media/hdd8T1/baseline/debug_cure_Wsmith 1800
        final_result_repr = f'{tester_repr}_{result_id}_{time_str}'
        ...
        cmd = f'python run_dir_tester_main.py no_mutation_halfdump {case_dir} {result_dir} {time}'
        print(f'The command is : \n\t{cmd}')
        print('The result will be saved in :\n\t', result_dir)
        # ; python script_extract_smith_case_name_from_exec_log.py  {result_dir}   mv_tested_cases {case_dir}  /media/hdd8T1/inv_wasm_smith/pre_seed/a_default_simplest_rewrite_tested_cur
    # elif tester
    else:

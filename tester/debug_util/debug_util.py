...
def get_log_by_container_impl(impl, wasm_path):
    if isinstance(impl, str):
        impl_name = impl
        impl = cp910_contianer_impls_dict[impl_name]
    assert isinstance(impl, uninstRuntime)
    log = impl.execute_and_collect_txt(wasm_path)
    return log




def get_log_by_lastest_impl(impl, wasm_path):
    if isinstance(impl, str):
        impl_name = impl
...
def is_executable_by_latest_impl(impl, wasm_path):
    log = get_log_by_lastest_impl(impl, wasm_path)
    return not is_failed_content(log)




def get_log_by_impl(impl, wasm_path):
    assert isinstance(impl, uninstRuntime)
    log = impl.execute_and_collect_txt(wasm_path)
    return log


def is_executable_by_impl(impl, wasm_path):
    log = get_log_by_impl(impl, wasm_path)

...
from ..InstModel.ValConstraint.EmptyValConstraint import EmptyValConstraint
...
def get_func_constraint(solve_info:ImSolveInfo)->SelectFuncConstraint:
    try:
        ...
        _context_condition_func = SelectFuncConstraintFactoryAnd.generate_a_combined_func_from_two_func_list(can_apply_funcs, pre_funcs)
    except OPTypeUndeterminedException as e:
        raise e
        _context_condition_func = SelectFuncConstraintFactory.get_default_func_constraint()
    return _context_condition_func
...
class RuleV2:
    def __init__(self, 
        ...
        self._context_condition_func = None
        if self.illegal_type:
            ...
            _context_condition_func: SelectFuncConstraint = get_func_constraint(raw_solve_info)
            self._context_condition_func = _context_condition_func
    ...
    @property
    def context_condition_func(self) -> SelectFuncConstraint:
        assert self._context_condition_func is not None
        return self._context_condition_func

    @context_condition_func.setter
    def context_condition_func(self, func:SelectFuncConstraint):
        self._context_condition_func = func

    @property
    def pre_func(self):
        if self._context_condition_func is None:
            return None
        return self._context_condition_func.pre_func

    @property
    def can_apply_func(self):
        if self._context_condition_func is None:
            return None
        return self._context_condition_func.can_apply_func
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}({self.raw_op_name}, {self.op}, {self.ph_env}, {self._context_condition_func}, {self.val_constraints})'

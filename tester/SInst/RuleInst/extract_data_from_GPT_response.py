...
class GptResponseData:
    def __init__(self,
        ...
        self.all_validate_constraints = self.validate_part.get('ContextConstraint', []) \
            + self.validate_part['InputConstraint']
    ...
    def always_valid(self):
        for constraint_dict in self.all_validate_constraints:
            if not self._is_trival_type_eq_valid_cs(constraint_dict):
                return False
        return True

    def _is_trival_type_eq_valid_cs(self, d):
        if d['v1'].endswith('.type') \
            and d['v2'] in val_type_strs \
                and d['relation'] == 'eq':
            return True
        return False
    ...
    def from_raw_response_file(cls, raw_file_path: Union[str, Path]):
        ...
        inst_name = get_inst_name_from_raw_file_path(raw_file_path)
...
def get_inst_name_from_raw_file_path(raw_file_path):

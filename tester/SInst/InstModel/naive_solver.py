...
class EqExprConstraintNaiveSolver(FiniteOneValConstraintSolver):
    ...
    def get_all_candis(c: ValConstraint, context: Context) -> set:
        ...
        val, cs = expr2.get_symbol_and_constraints(context=context, ph_env=PHEnv({}, 'xx'))
        return {val}
...
class LCExprConstraintNaiveSolver(FiniteOneValConstraintSolver):
    ...
    def _determin_max(c, context, expr2):
        max_val, cs = expr2.get_symbol_and_constraints(context=context, ph_env=None)
...
def _is_can_handle_gt(c:ValConstraint)->bool:
    if not isinstance(c, ExprConstraint):
        return False
    if c.relation not in {InstValRelation.GE, InstValRelation.GT, InstValRelation.LE, InstValRelation.LT}:
        return False
    expr1 = c.expr1
    expr2 = c.expr2
    if c.relation == InstValRelation.LE or c.relation == InstValRelation.LT:
...
def _get_concrete_gt_bar(c:ExprConstraint, context:Context):
    ...
    assert isinstance(expr1, OneValExpr)
    assert isinstance(expr2, OneValExpr)
    if c.relation == InstValRelation.GT or c.relation == InstValRelation.LT:
        offset = 1
    else:
        offset = 0
    if c.relation == InstValRelation.LE or c.relation == InstValRelation.LT:
        expr1, expr2 = expr2, expr1
    if not isinstance(expr1.val, (ImmPH, OperandPH)):
        raise Exception(f'Unimplemented for {c}, {type(expr1.val)}, {type(expr2.val)}, {expr1.val}, {expr2.val}, c is {c}')
    min_val, cs = expr2.get_symbol_and_constraints(context=context, ph_env=None)

...
from .SpecialContextConstVal.SpecialContextConstVal import SpecialContextConstVal, SpecialContextOneDefSize, SpecialContextOneVal
...
def get_a_mock_parser(context:Context) -> WasmParser:
    return WasmParser(
        types=context.types,
        imports=[],
        defined_func_ty_ids=context.func_type_idxs,
...
def get_pre_func(can_apply_func)->Optional[Callable]:
    if isinstance(can_apply_func, partial):
        ...
        if func == base_func_for_context_size_func:
            r_context_val:SpecialContextOneVal = keywards['r_context_val']
            ...
            if r_context_val.context_val_type == ContextValAttr.Locals:
                if relation in {InstValRelation.LT, InstValRelation.LE}:
                    def _func1(context:Context, insts):
                        concrete_context_cval = r_context_val.get_size_concrete_val(context=context)

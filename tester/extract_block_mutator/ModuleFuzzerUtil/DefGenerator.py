...
from ..SpecialConst import SpecialModuleConstGetter
...
def _valid_start_func_candis_candis(wasm_parser:WasmParser, skip_idxs=None) -> int:
    if skip_idxs is None:
        skip_idxs = []
    enpry_ty = funcTypeFactory.generate_one_func_type_default([], [])
    defined_func_idxs = [idx for idx, func in enumerate(wasm_parser.defined_funcs) if (func.func_ty == enpry_ty) and (idx not in skip_idxs)]
    if len(defined_func_idxs) == 0:
        raise ValueError(f'no valid start func: skip_idxs:{skip_idxs}; func_num: {wasm_parser.func_num} {startSecGenerator.can_generate_valid_one(wasm_parser)}')
...
class startSecGenerator:
    ...
    @staticmethod
    def can_generate_valid_one(wasm_parser:WasmParser):
        enpry_ty = funcTypeFactory.generate_one_func_type_default([], [])
        for func in wasm_parser.defined_funcs:
    ...
    @staticmethod
    def generate_valid_one(wasm_parser: WasmParser, skip_idxs=None):
        idx = startSecGenerator.idx_field.random_valid_cvalue(wasm_parser=wasm_parser, skip_idxs=skip_idxs)
        return idx

data_count_field = OneFuncValueField(FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurDataSegNum)))



def _get_length_for_active_data_sec(determined_offset, wasm_parser:WasmParser, memory_idx=0):
    mem_length = SpecialModuleConstGetter(SpecialModuleConst.CurMemLen).get_concrete_value(memory_idx=memory_idx, loader=wasm_parser)
...
active_data_offset_field = funcDeterminedRangeIntValueField(0, FieldFunc(func=SpecialModuleConstGetter(SpecialModuleConst.CurMemLen).get_concrete_value), False, False)
actual_length_field = funcDeterminedRangeIntValueField(0, FieldFunc(_get_length_for_active_data_sec), False, False)
mem_idx_field = funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurDefMemNum).get_concrete_value),False, True)
...
class activeDataDef0Gen(ValueField):
    ...
    def random_valid_cvalue(self, wasm_parser:WasmParser) -> Any:
        offset = active_data_offset_field.random_valid_cvalue(memory_idx=0, loader=wasm_parser)
...
class activeDataDef1Gen(ValueField):
    ...
    def random_valid_cvalue(self, wasm_parser:WasmParser) -> Any:
        memory_idx = mem_idx_field.random_valid_cvalue(loader=wasm_parser)
        offset = active_data_offset_field.random_valid_cvalue(memory_idx=memory_idx, loader=wasm_parser)
        actual_length = activeDataSegmentGenerator.actual_length_field.random_valid_cvalue(
            determined_offset=offset,
            wasm_parser=wasm_parser,
...
class activeDataSegmentGenerator:
    ...
    mem_idx_field = funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurDefMemNum).get_concrete_value),False, True)
    offset_fileld = funcDeterminedRangeIntValueField(0, FieldFunc(func=SpecialModuleConstGetter(SpecialModuleConst.CurMemLen).get_concrete_value), False, False)  #
    actual_length_field = funcDeterminedRangeIntValueField(0, FieldFunc(_get_length_for_active_data_sec), False, False)
    ...
    @staticmethod
    def can_insert(wasm_parser:WasmParser):
        return len(wasm_parser.defined_memory_datas) > 0
    
    @staticmethod
    def generate_valid_one(wasm_parser:WasmParser):
        def_ = activeDataSegmentGenerator.gen_main.random_valid_cvalue(wasm_parser=wasm_parser)
        return def_
...
_one_funcref_field_with_null = ComibedField(
    [
        funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurFuncNum).get_concrete_value), False, True),
        discreteValueField([None])
    ],
    [0.8, 0.2]
)
...
funcidx_field = funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurFuncNum).get_concrete_value), False, True)
funcidxs_field= ListedField(funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurFuncNum).get_concrete_value), False, True))
...
class passiveElemDef0Gen(ValueField):
    ...
    def random_valid_cvalue(self, *args, **kwds):
        ...
        elems = funcidxs_field.random_valid_cvalue(length=actual_length, loader=kwds.get('wasm_parser'))
...
class passiveElemDef1Gen(ValueField):
    ...
    def random_valid_cvalue(self, *args, **kwds):
        ...
        actual_length = unconstrain_elem_list_length_field.random_valid_cvalue()
        elems = elem_exprs_field.random_valid_cvalue(length=actual_length, key=ref_type, loader=kwds.get('wasm_parser'))
...
class passiveElemSegmentGenerator:
    ...
    @staticmethod
    def generate_valid_one(wasm_parser:WasmParser, ref_type:Optional[str]=None):
        if ref_type == 'externref':
            result =  passive_elem_gen1.random_valid_cvalue(wasm_parser=wasm_parser, ref_type=ref_type)
        else:
            result = passive_elem_gen.random_valid_cvalue(wasm_parser=wasm_parser)
        return result
...
class declarativeElemSegmentGenerator:
    ...
    def generate_valid_one(wasm_parser:WasmParser, ref_type:Optional[str]=None):
        if ref_type == 'externref':
            return decl_elem_gen1.random_valid_cvalue(wasm_parser=wasm_parser, ref_type=ref_type)
        return decl_elem_gen.random_valid_cvalue(wasm_parser=wasm_parser)


def _get_length_for_active_elem_sec(determined_offset, wasm_parser:WasmParser, table_idx=0):
    mem_length = SpecialModuleConstGetter(SpecialModuleConst.CurTableLength).get_concrete_value(table_idx=table_idx, loader=wasm_parser)
    rest_length = mem_length - determined_offset
    max_length = min(rest_length, SpecialDefConst.MaxOneElemLen.get_concrete_value())
    return max_length


def _determine_ref_type_field_for_table(wasm_parser:WasmParser, table_idx:int):
    table = wasm_parser.defined_table_datas[table_idx]
    return get_table_attr(table, 'val_type')


table_idx_field = funcDeterminedRangeIntValueField(0, FieldFunc(func=SpecialModuleConstGetter(SpecialModuleConst.CurDefTableNum).get_concrete_value), False, True)
...
class activeElemSegmentGenerator:
    ...
    offset_field = funcDeterminedRangeIntValueField(0, FieldFunc(func=SpecialModuleConstGetter(SpecialModuleConst.CurTableLength).get_concrete_value), False, False)
    elem_list_length_field = funcDeterminedRangeIntValueField(0, FieldFunc(_get_length_for_active_elem_sec), False, False)
    
    @staticmethod
    def can_insert(wasm_parser:WasmParser):
        return len(wasm_parser.defined_table_datas) > 0
    
    @staticmethod
    def generate_valid_one(wasm_parser:WasmParser, *args, **kwds):
        ...
        if 'tableidx' in required_filed_names:
            table_idx = table_idx_field.random_valid_cvalue(loader=wasm_parser)
            paras[type_repr2fields['tableidx']] = table_idx
        else:
            table_idx = 0
        if 'offset_expr' in required_filed_names:
            offset = activeElemSegmentGenerator.offset_field.random_valid_cvalue(table_idx=table_idx, loader=wasm_parser)
        ...
        if 'valtype' in required_filed_names:
            ref_type = activeElemSegmentGenerator.ref_type_field.random_valid_cvalue(table_idx=table_idx, wasm_parser=wasm_parser)
        ...
        if 'funcidxs' in required_filed_names:
            actual_length = activeElemSegmentGenerator.elem_list_length_field.random_valid_cvalue(
                ...
                wasm_parser=wasm_parser, 
                table_idx=table_idx)
            elems = funcidxs_field.random_valid_cvalue(length=actual_length, loader=wasm_parser)
        ...
        if 'exprs' in required_filed_names:
            actual_length = activeElemSegmentGenerator.elem_list_length_field.random_valid_cvalue(
                ...
                wasm_parser=wasm_parser, 
                table_idx=table_idx)
            elems = elem_exprs_field.random_valid_cvalue(length=actual_length, key=ref_type, loader=wasm_parser)
...
global_idx_field = funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurGlobalNum).get_concrete_value), False, True)
table_idx_field = funcDeterminedRangeIntValueField(0, FieldFunc(SpecialModuleConstGetter(SpecialModuleConst.CurDefTableNum).get_concrete_value), False, True)
...
class ExportFunidxGen:
    ...
    def random_valid_cvalue(self, wasm_parser:WasmParser):
        func_idx = funcidx_field.random_valid_cvalue(loader=wasm_parser)
...
class ExportMemidxGen:
    def __init__(self):
        pass
    def random_valid_cvalue(self, wasm_parser:WasmParser):
        mem_idx = mem_idx_field.random_valid_cvalue(loader=wasm_parser)
        return gen_export_memidx_part(mem_idx)
class ExportGlobalidxGen:
    def __init__(self):
        pass
    def random_valid_cvalue(self, wasm_parser:WasmParser):
        global_idx = global_idx_field.random_valid_cvalue(loader=wasm_parser)
...
class ExportIdxGen:
    ...
    def random_valid_cvalue(self, wasm_parser:WasmParser):
        generated_ones = []
        if SpecialModuleConstGetter(SpecialModuleConst.CurFuncNum).get_concrete_value(loader=wasm_parser) > 0:
            generated_ones.append(export_funidx_field.random_valid_cvalue(wasm_parser=wasm_parser))
        if SpecialModuleConstGetter(SpecialModuleConst.CurTableNum).get_concrete_value(loader=wasm_parser) > 0:
            generated_ones.append(export_tableidx_field.random_valid_cvalue(wasm_parser=wasm_parser))
        if SpecialModuleConstGetter(SpecialModuleConst.CurMemNum).get_concrete_value(loader=wasm_parser) > 0:
            generated_ones.append(export_memidx_field.random_valid_cvalue(wasm_parser=wasm_parser))
        if SpecialModuleConstGetter(SpecialModuleConst.CurGlobalNum).get_concrete_value(loader=wasm_parser) > 0:
            generated_ones.append(export_globalidx_field.random_valid_cvalue(wasm_parser=wasm_parser))

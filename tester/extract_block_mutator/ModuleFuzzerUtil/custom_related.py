...
def can_insrt_custom_section(wasm_parser: WasmParser) -> bool:
    return True


def insert_custom_section(wasm_parser: WasmParser):
    custom_section = generate_a_custom_section()
    wasm_parser.customs.append(custom_section)


def custom_section_exist(wasm_parser: WasmParser) -> bool:
    return len(wasm_parser.customs) > 0



def delete_custom_section(wasm_parser: WasmParser):
    wasm_parser.customs = []


def rewrite_custom_section(wasm_parser: WasmParser):
    

    custom_section = generate_a_custom_section()
    wasm_parser.customs.pop(-1)
    wasm_parser.customs.append(custom_section)

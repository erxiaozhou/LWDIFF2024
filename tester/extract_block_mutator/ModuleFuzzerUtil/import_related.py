...
def gen_import_desc_(wasm_parser: WasmParser):
    raise Exception('randomly generate import desc is not a good idea')
    ...
    import_attr = import_attr_field.random_valid_cvalue(key=import_attr_type, wasm_parser=wasm_parser)
        
    import_desc = gen_import_desc(import_module_name, import_entity_name, import_attr)
    return import_desc
    

def insert_import(wasm_parser: WasmParser):
    import_desc = gen_import_desc_(wasm_parser)
    wasm_parser.imports.append(import_desc)

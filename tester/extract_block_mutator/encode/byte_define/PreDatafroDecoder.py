...
def gen_part_desc_by_dicts(
    dicts:list[dict],
    to_ir_component_func:Optional[Callable]=None,
    ...
    new_to_ir_component_func, new_return_is_self_payload = get_p_to_ir_component_func(to_ir_component_func, datapayload_name, the_one_union_name)
    ...
    part_desc = PartDesc(
        ...
        to_ir_component_func=new_to_ir_component_func,
...
def get_p_to_ir_component_func(to_ir_component_func:Optional[Callable], datapayload_name:str, the_one_non_prefix_name:Optional[str])->tuple[Callable, bool]:
    if to_ir_component_func is None:
        actual_toactual_actual_mponent_func = return_para
    else:
        actual_toactual_actual_mponent_func = to_ir_component_func

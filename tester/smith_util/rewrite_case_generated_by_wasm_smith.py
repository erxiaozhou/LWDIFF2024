...
def _generate_entry_func(parser: WasmParser, longest_func_idx: int):
    ...
    return wasmFunc.from_text(func_fext, new_func_type)
...
class ByteSmithRewriter:
    ...
    def rewrite_main(self, p):
        ...
        func_type_idxs = function_sec.func_type_idxs
        type_sec = _typeSection.from_ba(self.sec_dict['type'])
        func_types = type_sec.types
        max_func_type = func_types[func_type_idxs[max_func_idx]]
        ...
        function_sec.func_type_idxs.append(type_idx)
...
class _FunctionSection:
    def __init__(self, func_type_idxs):
        self.func_type_idxs = func_type_idxs
    ...
    @property
    def as_bytes_without_length(self):
        type_idx_bytes = [leb128.u.encode(x) for x in self.func_type_idxs]
